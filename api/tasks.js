...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      const { data, error } = await sb.from('tasks').select('id,title,date,notes,completed');
      if (error) return res.status(500).json({ error: 'Failed to read tasks' });
    ...
    if (req.method === 'POST') {
      ...
      if (error) return res.status(500).json({ error: 'Failed to upsert tasks' });
